...
import { Card, COLOR, TYPE, RARITY } from '../src/Card.js';
import { CreatureCard } from '../src/CreatureCard.js';
import { PlanesWalkerCard } from '../src/PlanesWalkerCard.js';
import { Database } from '../src/Database.js';
import { App, inputCardModify } from '../src/App.js';
import { PlanesWalkerCardCreator, CardCreator } from '../src/CardsCreaters.js';
...
describe('Tests of the class Database', () => {
  let db : Database;
  let card : PlanesWalkerCard;
  beforeEach(() => {
    db = Database.getInstance();
    card = new PlanesWalkerCard( 1, 'cartita1', 2, COLOR.NOCOLOR, TYPE.PLANESWALKER, RARITY.COMMON, 'Esto hace cosas', 10, 5);
  });
  ...
  it('should return the cards of the user', () => {
    const cards : Card[] = db.getCardsbyUserName('test_user');
    expect(cards).to.be.an('array');
    expect(cards.length).to.equal(3);
    let card : Card = new CreatureCard(1, 'cartita1', 2, COLOR.NOCOLOR, TYPE.CREATURE, RARITY.COMMON, 'Esto hace cosas', 10, 2, 2);
    expect(cards[0]).to.eql(card);
    expect(cards[1].id).to.eql(2);
  });

  it('should add a card to the user', () => {
    db.writeCards('test_user2', [card]);
    const cards : Card[] = db.getCardsbyUserName('test_user2');
    expect(cards[0]).to.be.eql(card);
  });
});

describe('Tests of the class App', () => {
  let app : App;
  beforeEach(() => {
    app = new App('test_user3');
  });
  ...
  it('should return the cards of the user', () => {
    const cards : Card[] = app.getCards();
    expect(cards).to.be.an('array');
    expect(cards.length).to.equal(1);
    let card : Card = new CreatureCard(1, 'cartita1', 2, COLOR.NOCOLOR, TYPE.CREATURE, RARITY.COMMON, 'Esto hace cosas', 10, 2, 2);
    expect(cards[0]).to.eql(card);
    expect(cards[0].id).to.eql(1);
  });

  it('should add a card to the user', () => {
    const card : Card = new PlanesWalkerCard( 2, 'cartita2', 2, COLOR.NOCOLOR, TYPE.PLANESWALKER, RARITY.COMMON, 'Esto hace cosas', 10, 5);
    app.addCard(card);
    const cards : Card[] = app.getCards();
    expect(cards[1]).to.be.eql(card);
  });

  it('should remove a card from the user', () => {
    app.removeCard(2);
    const cards : Card[] = app.getCards();
    expect(cards.length).to.equal(1);
  });
  ...
  it('should return a PlanesWalkerCard object when type is "planeswalker"', () => {
    const result = new PlanesWalkerCardCreator(2, 'Test Planeswalker', 5, COLOR.BLUE, TYPE.PLANESWALKER, RARITY.MYTHIC, 'Test rules text', 20, 3).createCard();
    expect(result).to.be.an.instanceOf(PlanesWalkerCard);
    expect(result.id).to.equal(2);
  });
  
  it('should return a Card object for unknown type', () => {
    const result =  new CardCreator( 3, 'Test Card', 2, COLOR.GREEN, TYPE.ARTIFACT, RARITY.UNCOMMON, 'Test rules text', 15).createCard();
    expect(result).to.be.an.instanceOf(Card);
    expect(result.id).to.equal(3);
  });

  it('should modify the name of the card', () => {
    const card : Card = new PlanesWalkerCard(2, 'cartita2', 2, COLOR.NOCOLOR, TYPE.PLANESWALKER, RARITY.COMMON, 'Esto hace cosas', 10, 5);
    app.addCard(card);
    const modifyParams: inputCardModify = {
      name_: 'Modified Card Name'
    };
    app.modifyCard(2, modifyParams);
    const modifiedCard = app.getCards()[1];
    expect(modifiedCard.name).to.equal('Modified Card Name');
  });

  it('should modify the mana cost of the card', () => {
    const card : Card = new PlanesWalkerCard( 2, 'cartita2', 2, COLOR.NOCOLOR, TYPE.PLANESWALKER, RARITY.COMMON, 'Esto hace cosas', 10, 5);
    app.addCard(card);
    const modifyParams: inputCardModify = {
      mana_cost_: 5
    ...
    app.modifyCard(2, modifyParams);
    const modifiedCard = app.getCards()[1];
    expect(modifiedCard.manaCost).to.equal(5);
  });

  it('should modify the color of the card', () => {
    const card : Card = new PlanesWalkerCard( 2, 'cartita2', 2, COLOR.NOCOLOR, TYPE.PLANESWALKER, RARITY.COMMON, 'Esto hace cosas', 10, 5);
    app.addCard(card);
    const modifyParams: inputCardModify = {
      color_: 'blue'
    ...
    app.modifyCard(2, modifyParams);
    const modifiedCard = app.getCards()[1];
    expect(modifiedCard.color).to.equal(COLOR.BLUE);
  });

  it('should show the collection', () => {
    expect(app.showCollection()).to.be.undefined;
